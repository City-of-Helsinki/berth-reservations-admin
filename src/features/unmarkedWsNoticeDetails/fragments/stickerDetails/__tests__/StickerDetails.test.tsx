...
describe('StickerDetails', () => {
  it('renders correctly', () => {
    const wrapper = mount(
      <StickerDetails
        ...
        stickerPosted={null}
        handleAssignNewStickerNumber={jest.fn()}
      />
    );
  ...
  it('renders leading zeros', () => {
    const wrapper = mount(<StickerDetails stickerNumber={1} stickerPosted={null} stickerSeason={'2020/2021'} />);
    expect(wrapper.find(LabelValuePair).first().text()).toContain('0001');
  });

  it('renders dash when stickerNumber is null', () => {
    const wrapper = mount(<StickerDetails stickerNumber={null} stickerPosted={null} stickerSeason={'2020/2021'} />);
